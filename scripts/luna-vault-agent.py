...
def cmd_inbox(args: argparse.Namespace) -> int:
    """Read incoming requests addressed to this session.

    Non-destructive by default so a request is not lost if the agent crashes
    before replying. Pass --consume to delete after reading.
    """
    ensure_dirs()
    sid = session_id()
    inbox = INBOX_DIR / sid
    ...
    while True:
        ...
        if found:
            print(json.dumps([d for _, d in found], indent=2))
            if args.consume:
                for path, _ in found:
                    path.unlink(missing_ok=True)
            return 0
...
def cmd_reply(args: argparse.Namespace) -> int:
    """Send a response back to the session that sent a request."""
    ensure_dirs()
    sid = session_id()
    msg_id = str(uuid.uuid4())
    payload = {
    ...
    # Retire the request we just answered so it is not replied to twice.
    if args.id:
        own_inbox = INBOX_DIR / sid
        for path in own_inbox.glob("*.json") if own_inbox.exists() else []:
            try:
                data = json.loads(path.read_text())
            except json.JSONDecodeError:
                continue
            if data.get("type") == "request" and data.get("id") == args.id:
                path.unlink(missing_ok=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_inbox = sub.add_parser("inbox", help="read incoming requests for this session")
    p_inbox.add_argument("--from", dest="from_session")
    p_inbox.add_argument("--timeout", type=int, default=0)
    p_inbox.add_argument("--consume", action="store_true", help="delete requests after reading")

    p_reply = sub.add_parser("reply", help="respond to a request from another session")
    p_reply.add_argument("--to", required=True)
    p_reply.add_argument("--id", help="id of the request being answered")
    p_reply.add_argument("--status", required=True, choices=["done", "blocked", "rejected", "ack"])
